...
CREATE TABLE IF NOT EXISTS `pla_planet_post`(
    ...
    `summary`                   longtext NOT NULL COMMENT '帖子概要',
...
-- 星球画廊表
DROP TABLE IF EXISTS `pla_planet_gallery`;
CREATE TABLE IF NOT EXISTS `pla_planet_gallery`(
    `id`                        bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    `user_id`                   bigint(20) NOT NULL COMMENT '上传用户ID',
    `file_name`                 varchar(255) NOT NULL COMMENT '文件名称',
    `file_url`                  varchar(500) NOT NULL COMMENT '文件访问URL',
    `file_size`                 bigint(20) DEFAULT 0 COMMENT '文件大小(字节)',
    `file_type`                 varchar(50) NOT NULL DEFAULT 'image' COMMENT '文件类型',
    ...
    `year`                      int(11) DEFAULT NULL COMMENT '图片年份',
    ...
    INDEX `idx_created_at`(`created_at`) USING BTREE
) ENGINE = InnoDB CHARSET = utf8mb4 COMMENT = '星球画廊表' ROW_FORMAT = Dynamic;