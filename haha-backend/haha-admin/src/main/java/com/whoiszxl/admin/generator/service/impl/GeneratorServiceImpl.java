package com.whoiszxl.admin.generator.service.impl;
...
import com.whoiszxl.admin.generator.config.GeneratorProperties;
import com.whoiszxl.admin.generator.enums.QueryTypeEnum;
import com.whoiszxl.admin.generator.mapper.FieldConfigMapper;
import com.whoiszxl.admin.generator.mapper.GenConfigMapper;
...
import com.whoiszxl.starter.core.utils.TemplateUtils;
...
import com.whoiszxl.starter.mybatis.utils.Table;
import com.whoiszxl.starter.web.util.FileUploadUtils;
...
public class GeneratorServiceImpl implements GeneratorService {
    ...
    @Override
    public PageResponse<TableResp> pageTable(TableQuery query, PageQuery pageQuery) throws SQLException {
        ...
        tableList.removeIf(table -> StrUtil.equalsAnyIgnoreCase(table.getTableName(), generatorProperties
            .getExcludeTables()));
        CollUtil.sort(tableList, Comparator.comparing(Table::getCreatedAt)
            .thenComparing(table -> Optional.ofNullable(table.getUpdatedAt()).orElse(table.getCreatedAt()))
            .reversed());
        List<TableResp> tableRespList = BeanUtil.copyToList(tableList, TableResp.class);
        PageResponse<TableResp> pageResp = PageResponse.build(pageQuery.getPage(), pageQuery.getSize(), tableRespList);
    ...
    public GenConfigDO getGenConfig(String tableName) throws SQLException {
        ...
        if (null == genConfig) {
            ...
            GenConfigDO lastGenConfig = genConfigMapper.selectOne(Wrappers.lambdaQuery(GenConfigDO.class)
                .orderByDesc(GenConfigDO::getCreatedAt)
                .last("LIMIT 1"));
    ...
    public List<FieldConfigDO> listFieldConfig(String tableName, Boolean requireSync) {
        ...
        for (Column column : columnList) {
            ...
            // 更新已有字段配置
            if (null != fieldConfig.getCreatedAt()) {
                String columnType = StrUtil.splitToArray(column.getTypeName(), StringConstants.S_SPACE)[0].toLowerCase();
