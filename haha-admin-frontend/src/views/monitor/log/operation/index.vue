<template>
  ...
  >
    <template #custom-left>
      <a-input v-model="queryForm.createdByString" placeholder="请输入操作人" allow-clear @change="search">
    ...
    <template #consumingTime="{ record }">
      <a-tag v-if="record.consumingTime > 500" color="red">{{ record.consumingTime }}ms</a-tag>
      <a-tag v-else-if="record.consumingTime > 200" color="orange">{{ record.consumingTime }}ms</a-tag>
      <a-tag v-else color="green">{{ record.consumingTime }} ms</a-tag>
    </template>
  </GiTable>
...
const columns: TableInstanceColumns[] = [
  ...
  { title: '操作时间', slotName: 'createdAt', width: 180 },
  { title: '操作人', dataIndex: 'createdByString', ellipsis: true, tooltip: true },
  ...
  { title: '操作地点', dataIndex: 'ipAddress', ellipsis: true, tooltip: true },
  { title: '耗时', slotName: 'consumingTime', align: 'center' },
...
const reset = () => {
  ...
  queryForm.createdByString = undefined
  queryForm.createdAt = [
    dayjs().subtract(6, 'day').startOf('day').format('YYYY-MM-DD HH:mm:ss'),
    dayjs().endOf('day').format('YYYY-MM-DD HH:mm:ss')
  ]
