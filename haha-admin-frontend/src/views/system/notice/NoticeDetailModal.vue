<template>
  <a-modal v-model:visible="visible" :width="width >= 600 ? '70%' : '100%'" :footer="false" @close="reset">
        ...
        <div class="meta-data">
          <a-space>
            <span>
              ...
              <span>{{ dataDetail?.createdByString }}</span>
            </span>
            <a-divider direction="vertical" />
            <span>
              <icon-history class="icon" />
              <span class="label">发布时间：</span>
              <span>{{ dataDetail?.effectiveTime ? dataDetail?.effectiveTime : dataDetail?.createdAt }}</span>
            </span>
          </a-space>
        </div>
      </a-typography-paragraph>
    ...
    <a-divider />
    <div v-if="dataDetail?.updatedAt" class="update-time-row">
      <span>
        <icon-schedule class="icon" />
        <span>最后更新于：</span>
        <span>{{ dataDetail?.updatedAt }}</span>
      </span>
    </div>
  </a-modal>
</template>
