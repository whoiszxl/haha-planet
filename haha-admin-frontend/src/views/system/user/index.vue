      ...
      <a-col :xs="24" :sm="16" :md="17" :lg="18" :xl="19" :xxl="20" flex="1" class="h-full ov-hidden">
        ...
        >
          ...
          <template #custom-right>
            <a-button v-permission="['system:admin:add']" type="primary" @click="onAdd">
              <template #icon><icon-plus /></template>
              <span>新增</span>
            </a-button>
            <a-tooltip content="导出">
              <a-button v-permission="['system:admin:export']" class="gi_hover_btn-border" @click="onExport">
                <template #icon>
                  <icon-download />
                </template>
              </a-button>
          ...
          <template #action="{ record }">
            <a-space>
              <a-link v-permission="['system:admin:update']" @click="onUpdate(record)">修改</a-link>
              <a-link v-permission="['system:admin:delete']" status="danger"
                :title="record.isSystem ? '系统内置数据不能删除' : '删除'" :disabled="record.disabled" @click="onDelete(record)">
                删除
              </a-link>
              <a-dropdown>
                <a-link v-if="has.hasPermOr(['system:admin:resetPwd'])" type="text">更多</a-link>
                <template #content>
                  <a-doption v-permission="['system:admin:resetPwd']" @click="onResetPwd(record)">重置密码</a-doption>
                </template>
              </a-dropdown>
            </a-space>
          </template>
...
const columns: TableInstanceColumns[] = [
  ...
  { title: '昵称', dataIndex: 'nickname', width: 80, ellipsis: true, tooltip: true },
  { title: '状态', slotName: 'status', align: 'center', width: 80 },
  { title: '性别', slotName: 'gender', align: 'center', width: 80 },
  { title: '所属部门', dataIndex: 'deptName', ellipsis: true, tooltip: true, width: 120 },
  { title: '角色', dataIndex: 'roleNames', slotName: 'roleNames', width: 150 },
  { title: '手机号', dataIndex: 'mobile', width: 120, ellipsis: true, tooltip: true },
  { title: '邮箱', dataIndex: 'email', width: 150, ellipsis: true, tooltip: true },
  { title: '系统内置', slotName: 'isSystem', width: 100, align: 'center', show: false },
  { title: '创建时间', dataIndex: 'createdAt', width: 180 },
  { title: '修改时间', dataIndex: 'updatedAt', width: 180, show: false },
  {
    title: '操作',
    slotName: 'action',
    width: 200,
    ...
    show: has.hasPermOr(['system:admin:update', 'system:admin:delete', 'system:admin:resetPwd'])
  }
]
