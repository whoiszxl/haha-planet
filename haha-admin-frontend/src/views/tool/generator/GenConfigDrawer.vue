      ...
      <a-tab-pane key="1" title="生成配置">
        <a-form ref="formRef" :model="form" :rules="rules" class="gen-config" size="large">
          ...
          <a-form-item label="所属模块" field="moduleName">
            <a-input v-model="form.moduleName" placeholder="项目模块名称，例如：haha-admin" />
          </a-form-item>
          <a-form-item label="模块包名" field="packageName">
            <a-input v-model="form.packageName" placeholder="项目模块包名，例如：com.whoiszxl.admin.system" />
          </a-form-item>
            ...
            >
              <a-tooltip content="同步最新数据表结构">
                <a-button
                  ...
                  size="small"
                  title="同步"
                  :disabled="dataList.length !== 0 && dataList[0].createdAt == null"
                >
