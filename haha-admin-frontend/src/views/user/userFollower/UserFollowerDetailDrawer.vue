<template>
  <a-drawer v-model:visible="visible" title="用户粉丝(关注我的人)详情" :width="width >= 580 ? 580 : '100%'" :footer="false">
    <a-descriptions :column="2" size="large" class="general-description">
      <a-descriptions-item label="主键ID">{{ dataDetail?.id }}</a-descriptions-item>
      <a-descriptions-item label="用户ID(被关注者)">{{ dataDetail?.userId }}</a-descriptions-item>
      <a-descriptions-item label="关注者ID">{{ dataDetail?.followerId }}</a-descriptions-item>
...
import { type UserFollowerDetailResp, getUserFollower } from '@/apis'

const { width } = useWindowSize()

const visible = ref(false)
const dataId = ref('')
const dataDetail = ref<UserFollowerDetailResp>()
// 查询详情
const getDataDetail = async () => {
  const res = await getUserFollower(dataId.value)
